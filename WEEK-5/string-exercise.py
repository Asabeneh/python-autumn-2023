...
- spliting the text in two words
- count total number
- remove duplicates
- count unique number
- divide total unique words by total number of words
'''

# Donald J. Trump Presendiential Speech text
# Count total number of words:1259
# count toal number of unique words:576
# Find the lexical variety of the text in percentage:218.576 => 45.75

lowered_case_text = donald_speech.lower()
cleaned_text = lowered_case_text.replace('.', '').replace('’', '').replace(',',' ').replace('-', '').replace('_','').replace('–', '')
print(cleaned_text)
...
print('Total words', total_number_words)
print('Total unique words', total_number_unique_words)

word_variety = total_number_unique_words / total_number_words * 100
print('The word variety of the text is ', round(word_variety, 2))
